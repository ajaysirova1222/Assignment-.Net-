...
namespace Assignment.Net.services
{
    public class job
    {
        ...
        public async Task<responseData> GetResumeDetails(requestData req)
        {
            responseData resData = new responseData();
            resData.eventID = req.eventID;
            ...
            try
            {
                var adminUserId = req.addInfo["userId"]; 
                var JobPostedByUserId = "Admin";

                // Check if the user is an Admin
                var userCheckQuery = "SELECT RES.R_Id,RES.RESUME  FROM Resume RES INNER JOIN User U on U.UserId = RES.UserID where U.UserId =@userId ORDER BY RES.R_Id desc;";
                ...
                foreach (var userRow in userResult[0])
                {
                    var jobs = new Dictionary<string, object>
                    {
                        ["Id"] = userRow[0],  // UserID
                        ["File Name"] = userRow[1],    // Name
        ...
        public async Task<responseData> GetFileJobs(requestData req)
        {
            responseData resData = new responseData();
            resData.eventID = req.eventID;
